...
from directories.remove_tree_in_device import remove_tree
...
class Dir:
    ...
    def getting_a_list_of_directory_files(self, dir = True):
        if dir:
            if self.similar:
            ...
            n = DB_dir().find_files_in_dir(username=self.username, path=self.username)
            if len(n) == 0:
                return 'Directory is empty.'
            else:
                n = '\n'.join(n)
