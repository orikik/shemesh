...
class DB_dir:
    ...
    def dir_update(self, new_name, full_path):
        ...
        for path in mongo.dir_collection.find({'path' + m: n['name']}):
            old_path = path['full path']
            list = old_path.split('/')
            list[int(m)] = new_name
            new_full_path = '/'.join(list)
            mongo.dir_collection.update(path, {"$set": {"path" + m: new_name, 'full path': new_full_path}})
            for files in mongo.file_collection.find({'path': old_path}):
                mongo.file_collection.update(files, {"$set": {"path": new_full_path}})
        list = n['full path'].split('/')
        list[-1] = new_name
        new_full_path = '/'.join(list)
        for files in mongo.file_collection.find({'path': full_path}):
            mongo.file_collection.update(files, {"$set": {"path": new_full_path}})
        mongo.dir_collection.update(n, {"$set": {'name': new_name, 'full path': new_full_path}})
        for dev in mongo.dev_collection.find():
            Path().change_name_dir(dev=dev['path'], old_full_path=full_path, new_full_path=new_full_path)
